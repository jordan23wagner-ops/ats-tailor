...
// --- Payment/access gate --------------------------------------------------
// README previously claimed x402 USDC micropayments, but no payment
// verification was ever implemented - this endpoint was 100% open. Until
// real x402 verification is built, require a shared-secret header so it
// isn't a free, unmetered API. Set FLAGCHECK_API_KEY in Vercel and send it
// back as the x-api-key header on every request.
const providedKey = req.headers["x-api-key"];
  ...
  if (!expectedKey) {
    ...
    return respond(res, 500, { error: "Server misconfigured: access control not set up" });
  }
  if (!providedKey || providedKey !== expectedKey) {
    return respond(res, 401, { error: "Missing or invalid x-api-key header" });
  }
